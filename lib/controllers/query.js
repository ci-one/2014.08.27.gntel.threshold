...
//임계치 테스트용 요청
exports.updatefortest ="set @rev_seq = (select lpad(rev_seq + 1, 7,'0') from d_org_thr_rev order by rev_seq desc limit 1) COLLATE utf8_unicode_ci; insert into d_org_thr_rev values(?,?,?,?,@rev_seq,?,?,?,?,?,now(),?);";
...
//액션코멘트관리 등록현황 획득
exports.actList = 'select a.*, d.* from t_d_action a, t_d_actionData d where a.action_code=d.action_code';
//액션코멘트관리 액션코멘트 등록현황 획득
exports.actedList = 'select m.*,a.comm_name from t_m_action m,t_d_actionData a where m.comm_type=a.comm_type and m.is_delete is null;';
//액션코멘트관리 데이타 획득
exports.actData = 'select * from t_d_actionData;';
//액션코멘트 등록
exports.insertAct = 'insert into t_m_action values(?,?,?,?,?,null)';
//액션코멘트 업뎃
exports.updateAct = 'update t_m_action set comm_type=?,comm_file=?,comm_notice=? where kind=? and qlevel=? and is_delete is null';
//액션코멘트 삭제
exports.deleteAct = "update t_m_action set is_delete='y' where kind=? and qlevel=?";
